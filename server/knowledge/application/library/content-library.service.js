...
async function candidateRows(db,{section=null,sourceTitles=[]}={}){const params=[],where=[`c.exclude_from_knowledge=FALSE`];if(section){params.push(section);where.push(`c.metadata->>'section'=$${params.length}`)}if(sourceTitles.length){params.push(sourceTitles);where.push(`COALESCE(s.metadata->>'sourceFile',s.title)=ANY($${params.length}::text[])`)}const{rows}=await db.query(`SELECT c.id,c.atom_type::text AS "atomType",c.claim_type::text AS "claimType",c.candidate_text AS text,c.confidence,c.source_start AS "sourceStart",c.exclude_from_knowledge AS "excludeFromKnowledge",s.id AS "sourceId",COALESCE(s.metadata->>'sourceFile',s.title) AS "sourceTitle" FROM extraction_candidates c JOIN sources s ON s.id=c.source_id WHERE ${where.join(' AND ')} ORDER BY c.confidence DESC,c.source_start LIMIT 120`,params);return rows}
