...
function rawSections(node){return(node.sections||[]).map(value=>{const parts=String(value).split('::');return parts.length>1?parts.slice(1).join('::'):String(value)}).filter(Boolean)}
...
export async function buildContentLibraryDetail(db,nodeId){
 ...
 if(String(nodeId).startsWith('domain:')){
  ...
  return{ok:true,id:`domain:${domain.id}`,kind:'DOMAIN',label:domain.label,domainId:domain.id,domainLabel:domain.label,description:domain.description,keyPoints,relatedConcepts:[],sources,card:buildExtractiveCard(domain.label,keyPoints,sources,`domain:${domain.id}`),policy:{summaryMode:'extractive',canonicalTruthInvented:false,relationsRequireReview:true}};
 }
 const node=(map.nodes||[]).find(item=>item.id===nodeId&&item.kind==='SECTION_TOPIC');if(!node)return null;
 const domain=domainForSourceFiles(node.sourceFiles||[]),rows=await candidateRows(db,{sections:rawSections(node),sourceTitles:node.sourceFiles||[]}),keyPoints=selectKeyPoints(rows),sources=sourceViews(rows,(node.sourceFiles||[]).map(title=>({title})));
 return{ok:true,id:node.id,kind:'SECTION_TOPIC',label:node.label,domainId:domain?.id||null,domainLabel:domain?.label||'נושא שעדיין לא סווג',description:'המידע המוצג כאן נלקח מיחידות הידע שנמצאו תחת הכותרת הזאת במקורות השמורים.',keyPoints,relatedConcepts:[],sources,card:buildExtractiveCard(node.label,keyPoints,sources,node.id),policy:{summaryMode:'extractive',contextualGraphNeighborsHidden:true,relationsRequireReview:true,canonicalTruthInvented:false}};
}
