...
function submissionView(row,{includeText=false}={}){return{id:row.id,inputKind:row.input_kind,title:row.title,sourceUrl:row.source_url,fileName:row.file_name,mimeType:row.mime_type,textSha256:row.extracted_text_sha256,analysis:row.analysis,reviewStatus:row.review_status,decisionOverrides:row.decision_overrides,reviewedAt:row.reviewed_at,reviewedBy:row.reviewed_by,reviewNote:row.review_note,approvedSourceId:row.approved_source_id,createdAt:row.created_at,updatedAt:row.updated_at,...(includeText?{extractedText:row.extracted_text}:{})}}
export async function getIntakeSubmission(db,id,{includeText=false}={}){
 if(!await intakeSchemaReady(db))return null;const row=(await db.query(`SELECT * FROM intake_submissions WHERE id=$1`,[id])).rows[0];return row?submissionView(row,{includeText}):null;
}
export async function listIntakeSubmissions(db,{status='PENDING',limit=50}={}){
 ...
 return{schemaReady:true,items:rows.map(row=>submissionView(row))};
}
export async function changeIntakeSubmission(db,id,{reviewedBy,overrides={},note=''}){
 ...
 const row=(await db.query(`
  UPDATE intake_submissions SET decision_overrides=COALESCE(decision_overrides,'{}'::jsonb)||$2::jsonb,review_note=$3,updated_at=NOW()
  WHERE id=$1 AND review_status='PENDING' RETURNING *
 `,[id,JSON.stringify(overrides||{}),String(note||'').slice(0,10_000)])).rows[0];
...
export async function rejectIntakeSubmission(db,id,{reviewedBy,note=''}){
 ...
 const row=(await db.query(`
  UPDATE intake_submissions SET review_status='REJECTED',reviewed_at=NOW(),reviewed_by=$2,review_note=$3,updated_at=NOW()
  WHERE id=$1 AND review_status='PENDING' RETURNING *
 `,[id,reviewedBy,String(note||'').slice(0,10_000)])).rows[0];
...
async function persistSourceExtraction(client,sourceId){
 ...
 if(prior)return{skipped:true,runId:prior.id,summary:prior.stats};
 ...
 try{
  ...
  for(const candidate of candidates){
   ...
   const inserted=(await client.query(`
    ...
    ON CONFLICT(candidate_key,extractor_version) DO UPDATE SET run_id=EXCLUDED.run_id,updated_at=NOW() RETURNING id
   ...
   for(const edge of candidate.evidence)await client.query(`
    INSERT INTO extraction_candidate_evidence(candidate_id,fragment_id,source_start,source_end,fragment_start,fragment_end,exact_quote,exact_quote_verified)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8)
    ON CONFLICT(candidate_id,fragment_id,source_start,source_end) DO UPDATE SET fragment_start=EXCLUDED.fragment_start,fragment_end=EXCLUDED.fragment_end,exact_quote=EXCLUDED.exact_quote,exact_quote_verified=EXCLUDED.exact_quote_verified
   `,[inserted.id,edge.fragmentId,edge.sourceStart,edge.sourceEnd,edge.fragmentStart,edge.fragmentEnd,edge.quote,edge.exactQuoteVerified]);
  }
  await client.query('COMMIT');const summary=summarize(candidates);await client.query(`UPDATE extraction_runs SET status='COMPLETED',completed_at=NOW(),stats=$2::jsonb WHERE id=$1`,[runId,JSON.stringify(summary)]);return{skipped:false,runId,summary};
 }catch(error){try{await client.query('ROLLBACK')}catch{};try{await client.query(`UPDATE extraction_runs SET status='FAILED',completed_at=NOW(),error=$2 WHERE id=$1`,[runId,String(error?.stack||error).slice(0,10_000)])}catch{};throw error}
}
