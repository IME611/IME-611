...
function compactMatch(match){return{id:match.id,authority:match.authority,type:match.type,text:match.text,score:Number(safeNumber(match.score).toFixed(4)),sourceId:match.sourceId||null,sourceTitle:match.sourceTitle||null,sourceFile:match.sourceFile||null,section:match.section||null,metrics:{exact:Boolean(match.metrics?.exact),phrase:Boolean(match.metrics?.phrase),queryCoverage:Number(safeNumber(match.metrics?.queryCoverage).toFixed(4)),tokenContainment:Number(safeNumber(match.metrics?.tokenContainment).toFixed(4)),conflictSignal:Boolean(match.metrics?.conflictSignal)}}}
...
function placementAnalysis(atoms,map){
 ...
 const drawer=placements.find(item=>item.kind==='SECTION_TOPIC')||placements[0]||null;
 return{placements,suggestedDrawer:drawer?{nodeId:drawer.nodeId,kind:drawer.kind,label:drawer.label,confidence:Number(Math.min(.95,.42+drawer.score*.42+Math.min(.12,drawer.supportingAtoms*.02)).toFixed(4)),basis:'atomic-to-corpus-map lexical/context match'}:null};
}
...
export async function analyzeIntake(db,payload){
 ...
 return{ok:true,analysisVersion:'intake-v0.1',input:{kind:payload.kind,title:payload.title,sourceUrl:payload.sourceUrl||null,fileName:payload.fileName||null,mimeType:payload.mimeType,textCharacters:payload.text.length,textSha256:sha256(Buffer.from(payload.text,'utf8')),metadata:payload.metadata||{}},verdict:{...verdict,provisional:true,semanticModel:false},exactSourceMatch,atomic:{extractor:'atomic-he-v0.2',totalExtracted:rawAtoms.length,analyzed:atomResults.length,truncated:rawAtoms.length>atomResults.length,byType,byVerdict,atoms:atomResults.map(item=>({key:item.candidateKey,type:item.type,claimType:item.claimType,text:item.text,confidence:item.confidence,section:item.section,overlapVerdict:item.overlapVerdict,overlapConfidence:item.overlapConfidence,overlapReason:item.overlapReason,matches:item.matches}))},closestExistingKnowledge:strongestExisting(atomResults,whole),newMaterial:{count:byVerdict.NEW||0,sample:newAtoms},conflicts:{count:byVerdict.CONFLICTS||0,sample:conflictAtoms},placement,relations:{existingNeighborhood:relations,note:'These are evidence-linked relations already present around suggested corpus-map locations. They are not newly asserted relations from the submitted material.'},learning,corpusIndex:index.counts,decision:{required:true,allowed:['APPROVE','CHANGE','REJECT'],default:'REVIEW',note:'Analysis is staging-only. No source, concept, taxonomy, relation, or curriculum write occurs until explicit approval.'},policy:{canonicalWrites:false,autoMerge:false,semanticMatching:false,note:'Deterministic lexical/context analysis is intentionally conservative. Ambiguous synonym/paraphrase matches remain UNCERTAIN until semantic or creator review.'}};
}
