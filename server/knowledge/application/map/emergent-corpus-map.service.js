...
export async function buildEmergentCorpusMapPreview(db,{communityLimit=30,nodeLimit=150,edgeLimit=250,learnerVisibleOnly=false}={}){
 const visibility=learnerVisibleOnly?`AND (COALESCE((c.metadata->>'intakeApprovedSource')::boolean,FALSE)=FALSE OR COALESCE((c.metadata->>'learnerPublished')::boolean,FALSE)=TRUE)`:'';
 const conceptRows=(await db.query(`SELECT c.id,c.candidate_text,c.source_id,c.source_start,c.metadata->>'section' AS section,s.metadata->>'sourceFile' AS source_file FROM extraction_candidates c JOIN sources s ON s.id=c.source_id WHERE c.atom_type='CONCEPT' AND c.review_status<>'REJECTED' AND NOT c.exclude_from_knowledge ${visibility} ORDER BY c.source_id,c.source_start`)).rows;
