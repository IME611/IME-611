...
function mapLabelFromSection(value){
 return String(value||'').trim().replace(/^פרק\s+\d+\s*[:—–-]\s*/u,'').replace(/:\s*$/u,'').trim();
}
...
function buildConceptNodes(conceptRows){
 ...
 for(const row of conceptRows){
  ...
  const node=byNormalized.get(normalized)||{
   id:sha256(`concept:${normalized}`),kind:'CONCEPT',normalized,labels:new Map(),rawLabels:new Map(),candidateIds:[],sourceIds:new Set(),sourceFiles:new Set(),sections:new Set(),mappedAtomIds:new Set(),contextAtomIds:new Set(),
  };
  node.labels.set(mapLabel,(node.labels.get(mapLabel)||0)+1);node.rawLabels.set(row.candidate_text,(node.rawLabels.get(row.candidate_text)||0)+1);
  node.candidateIds.push(row.id);node.sourceIds.add(row.source_id);if(row.source_file)node.sourceFiles.add(row.source_file);
  if(row.section)node.sections.add(sourceSectionKey(row.source_id,row.section));byNormalized.set(normalized,node);
 }
...
function buildSectionTopicNodes(atomRows){
 const byNormalized=new Map(),genericCounts=new Map(),rawSectionAtomIds=new Set(),meaningfulSectionAtomIds=new Set();
 for(const row of atomRows){
  ...
  if(isGenericSection(label)){
   const key=normalizeKnowledgeText(label);genericCounts.set(key,{label,count:(genericCounts.get(key)?.count||0)+(row.atom_type==='CONCEPT'?0:1)});continue;
  }
  const normalized=normalizeKnowledgeText(label);if(!normalized)continue;
  const node=byNormalized.get(normalized)||{id:sha256(`section-topic:${normalized}`),kind:'SECTION_TOPIC',normalized,labels:new Map(),candidateIds:[],sourceIds:new Set(),sourceFiles:new Set(),sections:new Set(),mappedAtomIds:new Set(),contextAtomIds:new Set()};
  node.labels.set(label,(node.labels.get(label)||0)+1);node.sourceIds.add(row.source_id);if(row.source_file)node.sourceFiles.add(row.source_file);node.sections.add(sourceSectionKey(row.source_id,row.section));
  if(row.atom_type!=='CONCEPT'){node.contextAtomIds.add(row.id);meaningfulSectionAtomIds.add(row.id)}
 ...
 return{nodes,rawSectionAtomIds,meaningfulSectionAtomIds,genericSections:[...genericCounts.values()].filter(item=>item.count>0).sort((a,b)=>b.count-a.count||a.label.localeCompare(b.label,'he'))};
}

function addEdge(edges,a,b,{weight,signal,sourceId=null,section=null,atomId=null}){
 if(!a||!b||a.id===b.id)return;
 const key=pairKey(a.id,b.id),edge=edges.get(key)||{id:sha256(`edge:${key}`),from:a.id<b.id?a.id:b.id,to:a.id<b.id?b.id:a.id,weight:0,signals:{SOURCE_CONTEXT:0,SECTION_CONTEXT:0,SECTION_MEMBERSHIP:0,CO_MENTION:0},sourceIds:new Set(),sections:new Set(),atomIds:new Set()};
...
function buildEdges(conceptNodes,sectionNodes,atomRows){
 const edges=new Map(),conceptsBySource=new Map(),conceptsBySection=new Map(),sectionNodeBySection=new Map();
 for(const node of conceptNodes){
  for(const sourceId of node.sourceIds){const list=conceptsBySource.get(sourceId)||new Map();list.set(node.id,node);conceptsBySource.set(sourceId,list)}
  for(const sectionKey of node.sections){const list=conceptsBySection.get(sectionKey)||new Map();list.set(node.id,node);conceptsBySection.set(sectionKey,list)}
 }
 ...
 for(const[sectionKey,map]of conceptsBySection){
  const[sourceId,...rest]=sectionKey.split('::'),section=rest.join('::'),concepts=[...map.values()];
  pairs(concepts,(a,b)=>addEdge(edges,a,b,{weight:1.2,signal:'SECTION_CONTEXT',sourceId,section}));
  const sectionNode=sectionNodeBySection.get(sectionKey);if(sectionNode)for(const concept of concepts)addEdge(edges,sectionNode,concept,{weight:2.8,signal:'SECTION_MEMBERSHIP',sourceId,section});
 }
 ...
 for(const atom of atomRows){
  ...
  const sectionKey=atom.section?sourceSectionKey(atom.source_id,atom.section):null,sectionNode=sectionKey?sectionNodeBySection.get(sectionKey):null,contextConcepts=sectionKey?conceptsBySection.get(sectionKey):null;
  const hasContext=Boolean(sectionNode||contextConcepts?.size);
...
function buildCommunities(nodes,edges,{strongThreshold=1.2}={}){
 ...
 const degree=new Map(nodes.map(node=>[node.id,(adjacency.get(node.id)||[]).reduce((sum,item)=>sum+item.weight,0)])),byId=new Map(nodes.map(node=>[node.id,node])),ordered=[...nodes].sort((a,b)=>(degree.get(b.id)||0)-(degree.get(a.id)||0)||a.label.localeCompare(b.label,'he'));
 const assigned=new Set(),communities=[];
 for(const seed of ordered){
  if(assigned.has(seed.id))continue;const members=[seed];assigned.add(seed.id);
  ...
  let changed=true;while(changed&&members.length<30){
   changed=false;const memberIds=new Set(members.map(member=>member.id)),candidates=new Map();
   for(const member of members)for(const relation of adjacency.get(member.id)||[]){if(assigned.has(relation.nodeId)||memberIds.has(relation.nodeId))continue;const stat=candidates.get(relation.nodeId)||{weight:0,links:0};stat.weight+=relation.weight;stat.links+=1;candidates.set(relation.nodeId,stat)}
  ...
  const ranked=[...members].sort((a,b)=>(degree.get(b.id)||0)-(degree.get(a.id)||0)||a.label.localeCompare(b.label,'he')),sourceIds=new Set(),sourceFiles=new Set(),sections=new Set();
  ...
  const centralConcepts=ranked.slice(0,5).map(node=>({id:node.id,kind:node.kind,label:node.label,weightedDegree:Number((degree.get(node.id)||0).toFixed(3)),sourceCount:node.sourceCount}));
  const sectionCenter=centralConcepts.find(item=>item.kind==='SECTION_TOPIC'),derivedParts=sectionCenter?[sectionCenter,...centralConcepts.filter(item=>item.id!==sectionCenter.id)].slice(0,3):centralConcepts.slice(0,3);
...
export function buildEmergentCorpusMap({conceptRows,atomRows}){
 const conceptNodes=buildConceptNodes(conceptRows),sectionData=buildSectionTopicNodes(atomRows),sectionNodes=sectionData.nodes,nodes=[...conceptNodes,...sectionNodes];
 const{edges,explicitMappedAtoms,contextualAtoms,connectedAtoms,unmappedSectionCounts,noExplicitSectionCounts}=buildEdges(conceptNodes,sectionNodes,atomRows),edgeViews=edges.map(edgeView).sort((a,b)=>b.weight-a.weight||a.id.localeCompare(b.id)),communities=buildCommunities(nodes,edges);
 const knowledgeAtoms=atomRows.filter(row=>row.atom_type!=='CONCEPT').length,collapsedConceptCandidates=conceptRows.length-conceptNodes.length,strongEdges=edgeViews.filter(edge=>edge.weight>=1.2).length;
 const nodeViews=nodes.map(node=>({id:node.id,kind:node.kind,label:node.label,normalized:node.normalized,candidateCount:node.candidateCount,sourceCount:node.sourceCount,sourceFiles:asArray(node.sourceFiles),sections:asArray(node.sections),explicitMappedAtomCount:node.mappedAtomIds.size,contextAtomCount:node.contextAtomIds.size,rawCandidateVariants:node.rawCandidateVariants})).sort((a,b)=>b.contextAtomCount-a.contextAtomCount||b.explicitMappedAtomCount-a.explicitMappedAtomCount||b.sourceCount-a.sourceCount||a.label.localeCompare(b.label,'he'));
 return{
  ok:true,method:{version:'corpus-map-v0.3',basis:['map-labels-derived-from-exact-concept-candidates','source-observed-section-topics','same-source-context','same-section-context','section-concept-membership','explicit-concept-co-mention'],semanticModel:false,partition:'hub-neighborhood-preview'},
  summary:{conceptCandidates:conceptRows.length,conceptNodes:conceptNodes.length,mapCollapsedConceptCandidates:collapsedConceptCandidates,sectionTopicNodes:sectionNodes.length,totalMapNodes:nodes.length,knowledgeAtoms,rawSectionContextAtoms:sectionData.rawSectionAtomIds.size,rawSectionCoveragePercent:percentage(sectionData.rawSectionAtomIds.size,knowledgeAtoms),meaningfulSectionAtoms:sectionData.meaningfulSectionAtomIds.size,meaningfulSectionCoveragePercent:percentage(sectionData.meaningfulSectionAtomIds.size,knowledgeAtoms),explicitMappedAtoms,explicitCoveragePercent:percentage(explicitMappedAtoms,knowledgeAtoms),contextualAtoms,contextCoveragePercent:percentage(contextualAtoms,knowledgeAtoms),connectedAtoms,connectedCoveragePercent:percentage(connectedAtoms,knowledgeAtoms),trulyUnmappedAtoms:knowledgeAtoms-connectedAtoms,edges:edgeViews.length,strongEdges,communities:communities.length,singletonCommunities:communities.filter(community=>community.size===1).length},
  nodes:nodeViews,edges:edgeViews,communities,genericSections:sectionData.genericSections,unmappedSections:sectionCountView(unmappedSectionCounts),noExplicitMentionSections:sectionCountView(noExplicitSectionCounts),
  policy:{canonicalWrites:false,derivedLabels:true,note:'SECTION_TOPIC nodes come only from headings observed in source structure; generic structural headings are excluded from taxonomy-like nodes. Map labels may shorten descriptive concept candidate lines while raw candidate wording remains retained. Communities remain preview structures, not approved taxonomy.'},
 };
}

export async function buildEmergentCorpusMapPreview(db,{communityLimit=30,nodeLimit=150,edgeLimit=250}={}){
 const conceptRows=(await db.query(`SELECT c.id,c.candidate_text,c.source_id,c.metadata->>'section' AS section,s.metadata->>'sourceFile' AS source_file FROM extraction_candidates c JOIN sources s ON s.id=c.source_id WHERE c.atom_type='CONCEPT' AND c.review_status<>'REJECTED' AND NOT c.exclude_from_knowledge ORDER BY c.created_at,c.source_start`)).rows;
 const atomRows=(await db.query(`SELECT c.id,c.atom_type::text,c.candidate_text,c.source_id,c.metadata->>'section' AS section,s.metadata->>'sourceFile' AS source_file FROM extraction_candidates c JOIN sources s ON s.id=c.source_id WHERE c.review_status<>'REJECTED' AND NOT c.exclude_from_knowledge ORDER BY c.created_at,c.source_id,c.source_start`)).rows;
