...
function compactSpan(segment,{side,maxTokens=14}={}){
 const tokens=(normalizeKnowledgeText(segment).match(/[\p{L}\p{N}]+/gu)||[]).filter(Boolean);
 if(!tokens.length)return'';
 const chosen=side==='LEFT'?tokens.slice(-maxTokens):tokens.slice(0,maxTokens);
 return chosen.join(' ').trim();
}

function mappedEndpoint(found){
 if(!found)return null;
 return{nodeId:found.node.id,kind:found.node.kind,label:found.node.label,matchMode:found.mention.mode,resolutionStatus:'MAPPED',confidence:found.mention.score};
}

function unresolvedEndpoint(segment,{side,atom}){
 const label=compactSpan(segment,{side});
 if(!label)return null;
 return{
  nodeId:sha256(['SOURCE_SPAN',atom.source_id||'',atom.id||'',side,label].join('|')),
  kind:'SOURCE_SPAN',label,matchMode:'SOURCE_SPAN',resolutionStatus:'UNRESOLVED',confidence:.62,
 };
}

function relationKey(atomId,fromNode,toNode,type,version){return sha256([atomId,fromNode.nodeId,toNode.nodeId,type,version].join('|'))}
function endpointResolution(from,to){
 const mapped=(from.resolutionStatus==='MAPPED'?1:0)+(to.resolutionStatus==='MAPPED'?1:0);
 return mapped===2?'MAPPED':mapped===1?'PARTIAL':'UNRESOLVED';
}

export function extractExplicitRelations({nodes,atoms,version='relations-he-v0.2'}){
 ...
 for(const atom of atoms){
  ...
  for(const hit of hits){
   ...
   let from=mappedEndpoint(bestEntity(left,nodes,{side:'LEFT',sourceFile:atom.source_file}))||unresolvedEndpoint(left,{side:'LEFT',atom});
   let to=mappedEndpoint(bestEntity(right,nodes,{side:'RIGHT',sourceFile:atom.source_file}))||unresolvedEndpoint(right,{side:'RIGHT',atom});
   if(!from){skipped.missingLeft+=1;continue}
   if(!to){if(hit.pattern.pedagogic)skipped.pedagogicWithoutPair+=1;else skipped.missingRight+=1;continue}
   if(hit.pattern.reverse)[from,to]=[to,from];
   if(from.nodeId===to.nodeId){skipped.sameEndpoint+=1;continue}
   const resolution=endpointResolution(from,to),endpointConfidence=Math.min(from.confidence,to.confidence);
   const resolutionFactor=resolution==='MAPPED'?1:resolution==='PARTIAL'?.86:.72;
   const confidence=Math.min(.99,hit.pattern.base*endpointConfidence*resolutionFactor);
   relations.push({
    relationKey:relationKey(atom.id,from,to,hit.pattern.type,version),relationType:hit.pattern.type,
    from:{nodeId:from.nodeId,kind:from.kind,label:from.label,matchMode:from.matchMode,resolutionStatus:from.resolutionStatus},
    to:{nodeId:to.nodeId,kind:to.kind,label:to.label,matchMode:to.matchMode,resolutionStatus:to.resolutionStatus},
    endpointResolution:resolution,unresolvedEndpointCount:(from.resolutionStatus==='UNRESOLVED'?1:0)+(to.resolutionStatus==='UNRESOLVED'?1:0),
 ...
 const byType={},byResolution={};for(const relation of result){byType[relation.relationType]=(byType[relation.relationType]||0)+1;byResolution[relation.endpointResolution]=(byResolution[relation.endpointResolution]||0)+1}
 return{relations:result,summary:{total:result.length,byType,byResolution,skipped}};
}
...
export async function previewExplicitRelations(db,{limit=200}={}){
 const{conceptRows,atoms}=await loadMapInputs(db),map=buildEmergentCorpusMap({conceptRows,atomRows:atoms}),extracted=extractExplicitRelations({nodes:map.nodes,atoms,version:'relations-he-v0.2'});
 return{ok:true,mode:'preview',extractor:{method:'explicit-linguistic-patterns',version:'relations-he-v0.2',semanticModel:false},map:{nodes:map.summary.totalMapNodes,connectedCoveragePercent:map.summary.connectedCoveragePercent},summary:extracted.summary,relations:extracted.relations.slice(0,Math.max(1,Math.min(1000,Number(limit)||200))),policy:{canonicalWrites:false,requiresReview:true,note:'Only explicit linguistic cues produce typed relation candidates. Known Corpus Map nodes are mapped; unmatched endpoint text remains SOURCE_SPAN / UNRESOLVED for creator review and is never promoted automatically.'}};
}
