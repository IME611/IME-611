 ...
 async getSnapshot(){
  const [insights,experiments,reflections,counts]=await Promise.all([
   this.db.query(`SELECT id,statement,status::text AS status,evidence_strength,created_at
     FROM insights
     WHERE COALESCE((metadata->>'verification')::boolean,false)=false
     ORDER BY CASE status WHEN 'SUPPORTED' THEN 0 WHEN 'CHALLENGED' THEN 1 ELSE 2 END,created_at DESC LIMIT 5`),
   this.db.query(`SELECT e.id,e.insight_id,e.hypothesis,e.action,e.expected_signal,e.status::text AS status,e.started_at,e.ended_at,e.created_at,i.statement AS insight_statement,i.status::text AS insight_status
     ...
     WHERE COALESCE((i.metadata->>'verification')::boolean,false)=false
   ...
   this.db.query(`SELECT (SELECT COUNT(*)::int FROM sources) AS sources,(SELECT COUNT(*)::int FROM source_fragments) AS fragments,(SELECT COUNT(*)::int FROM claims WHERE COALESCE((metadata->>'verification')::boolean,false)=false) AS claims,(SELECT COUNT(e.id)::int FROM evidence e JOIN claims c ON c.id=e.claim_id WHERE COALESCE((c.metadata->>'verification')::boolean,false)=false) AS evidence`),
  ]);
