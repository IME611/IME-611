...
CREATE TABLE IF NOT EXISTS derived_knowledge (
  id BIGSERIAL PRIMARY KEY,
  source_document_id BIGINT NOT NULL REFERENCES source_documents(id) ON DELETE CASCADE,
  kind TEXT NOT NULL CHECK(kind IN ('SUMMARY','CATEGORY','TOPIC','INSIGHT','MATCH','EXTENSION','GAP','CONFLICT','NEW')),
  content TEXT NOT NULL,
  category_id TEXT,
...
CREATE INDEX IF NOT EXISTS derived_knowledge_source_idx ON derived_knowledge(source_document_id);
CREATE INDEX IF NOT EXISTS derived_knowledge_kind_idx ON derived_knowledge(kind);
...
CREATE TABLE IF NOT EXISTS knowledge_reviews (
  ...
  chapter_id INTEGER,category_id TEXT,score NUMERIC(8,3) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS knowledge_topics(id BIGSERIAL PRIMARY KEY,item_id BIGINT REFERENCES knowledge_items(id) ON DELETE CASCADE,topic TEXT NOT NULL,confidence NUMERIC(6,3) NOT NULL DEFAULT 0,UNIQUE(item_id,topic));
