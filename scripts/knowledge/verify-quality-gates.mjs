...
try{
 ...
 assert.equal(checks.migrationsApplied,expectedMigrations.length,'all canonical migrations 001-008 must be recorded');
 for(const name of ['candidatesWithoutEvidence','unverifiedCandidateEvidence','candidateSourceQuoteMismatch','evidenceFragmentQuoteMismatch','evidenceSourceMismatch','claimsWithoutEvidence','approvedRelationsWithUnresolvedEndpoints','nonExplicitRelationEvidence','relationSourceAtomMismatch','reviewDecisionsMissingReviewer','orphanExtractionReviewDecisions','orphanRelationReviewDecisions','pendingIntakeWithCanonicalSource','rejectedIntakeWithCanonicalSource','approvedIntakeWithoutCanonicalSource','migrationChecksumShapeFailures'])assert.equal(checks[name],0,`${name} must be zero`);

 console.log(JSON.stringify({ok:true,phase:'CORPUS_QUALITY_GATES_V0_1',checks,policy:{everyCandidateTraceable:true,exactQuotesVerified:true,canonicalClaimsEvidenceBacked:true,unresolvedRelationsCannotBeApproved:true,reviewDecisionsAttributedAndReferential:true,intakeReviewGatePreserved:true,migrationLedgerVerified:true}},null,2));
