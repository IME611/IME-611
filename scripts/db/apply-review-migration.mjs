...
const MIGRATION='database/migrations/008_review_decisions.sql';
...
try{
 await client.connect();await client.query("SELECT pg_advisory_lock(hashtext('eil_live_migrations'))");
 ...
 if(existing){if(existing.checksum!==checksum)throw new Error(`Migration drift detected for ${MIGRATION}`);console.log(`SKIP ${MIGRATION} (already applied)`)}else{console.log(`APPLY ${MIGRATION}`);await client.query(sql);await client.query('INSERT INTO schema_migrations(name,checksum) VALUES($1,$2)',[MIGRATION,checksum]);console.log(`OK ${MIGRATION}`)}
}finally{try{await client.query("SELECT pg_advisory_unlock(hashtext('eil_live_migrations'))")}catch{};await client.end()}
