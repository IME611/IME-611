...
export function loadLearningProgress(path:LearningPath):LearningProgressState{
  try{
    ...
    if(raw){
      ...
      if(parsed?.schemaVersion===1&&parsed.learningPathId===path.id&&parsed.learningPathVersion===path.version){
        return parsed;
      }
    }

    // One-time backward compatibility with the original numeric journey progress.
    ...
    state.activeStageId=path.stages.find(stage=>stage.order===legacy)?.id??path.stages.at(-1)?.id;

    for(const stage of path.stages){
      const chapter=stage.order;
      const reflection=localStorage.getItem(`eil-chapter-reflection-${chapter}`)?.trim();
      if(reflection){
        const now=new Date().toISOString();
        state.reflections[stage.id]={stageId:stage.id,text:reflection,createdAt:now,updatedAt:now};
      }
    }
    saveLearningProgress(path,state);
    return state;
...
export function saveLearningProgress(path:LearningPath,state:LearningProgressState){
  try{localStorage.setItem(keyFor(path),JSON.stringify(state))}catch{}
}

export function resetLearningProgress(path:LearningPath){
