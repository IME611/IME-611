...
export function DesktopNavigation({page,onNavigate,onAdd,collapsed,onCollapsedChange,online}:Props){
 ...
 const navigation=navigationForMode(owner),compact=owner?collapsed:true;
 return <GlassNavigation className={'productNav '+(compact?'collapsed':'')} aria-label="ניווט ראשי"><header><button className="navBrand" onClick={()=>onNavigate('dashboard')} aria-label="חזרה לדשבורד">E</button>{!compact&&<div><b>E.I.L</b><small>CREATOR MODE</small></div>}{owner&&<button className="navCollapse" onClick={()=>onCollapsedChange(!collapsed)} aria-expanded={!collapsed} aria-label={collapsed?'פתח סרגל':'כווץ סרגל'}>{collapsed?'‹':'›'}</button>}</header>{owner&&<button className="navAdd" onClick={onAdd}><span aria-hidden="true">＋</span>{!compact&&<div><b>הוסף מקור</b><small>מסמך · מאמר · מחשבה</small></div>}</button>}<nav aria-label="מסלולי E.I.L">{navigation.primary.map(item=><NavItem key={item.id}{...item} page={page} collapsed={compact} onNavigate={onNavigate}/>)}{navigation.groups.map(group=><section key={group.id} aria-label={group.label}>{!compact&&<h2>{group.label}</h2>}{group.items.map(item=><NavItem key={item.id}{...item} page={page} collapsed={compact} onNavigate={onNavigate}/>)}</section>)}</nav><footer aria-live="polite"><span className={online?'statusDot online':'statusDot'} aria-hidden="true"/>{!compact&&<small>{online?'מחובר למקורות':'מצב מקומי'}</small>}</footer></GlassNavigation>
}
