...
export function DesktopNavigation({page,onNavigate,onAdd,collapsed,onCollapsedChange,online}:Props){
 const owner=read('eil-access-mode','journey')==='owner';
 return <GlassNavigation className={'productNav '+(collapsed?'collapsed':'')} aria-label="ניווט ראשי"><header><button className="navBrand" onClick={()=>onNavigate('dashboard')} aria-label="חזרה לדשבורד">E</button>{!collapsed&&<div><b>E.I.L</b><small>{owner?'CREATOR MODE':'THE JOURNEY'}</small></div>}<button className="navCollapse" onClick={()=>onCollapsedChange(!collapsed)} aria-expanded={!collapsed} aria-label={collapsed?'פתח סרגל':'כווץ סרגל'}>{collapsed?'‹':'›'}</button></header>{owner&&<button className="navAdd" onClick={onAdd}><span aria-hidden="true">＋</span>{!collapsed&&<div><b>הוסף מקור</b><small>מסמך · מאמר · מחשבה</small></div>}</button>}<nav aria-label="מסלולי E.I.L">{primaryNavigation.map(item=><NavItem key={item.id}{...item} page={page} collapsed={collapsed} onNavigate={onNavigate}/>)}{mentalModelNavigation.map(group=>{const items=group.items;return items.length?<section key={group.id} aria-label={group.label}>{!collapsed&&<h2>{group.label}</h2>}{items.map(item=><NavItem key={item.id}{...item} page={page} collapsed={collapsed} onNavigate={onNavigate}/>)}</section>:null})}</nav><footer aria-live="polite"><span className={online?'statusDot online':'statusDot'} aria-hidden="true"/>{!collapsed&&<small>{online?'מחובר למקורות':'מצב מקומי'}</small>}</footer></GlassNavigation>
}

export function MobileNavigation({page,onNavigate,onAdd,online}:Omit<Props,'collapsed'|'onCollapsedChange'>){
 const[open,setOpen]=useState(false),owner=read('eil-access-mode','journey')==='owner';
