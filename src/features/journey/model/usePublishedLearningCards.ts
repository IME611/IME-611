...
function normalizeCards(value:unknown):LearningCard[]{
 ...
 return value.flatMap((row:any,index)=>{
  ...
  return[{id:String(row.id),order:index+1,type,title:String(row.title),text:String(row.text),sourceUnitIds:Array.isArray(row.sourceUnitIds)?row.sourceUnitIds.map(String):[],sourceId:row.sourceId?String(row.sourceId):undefined,sourceLabel:row.sourceLabel?String(row.sourceLabel):undefined,provenanceLabel:row.provenanceLabel?String(row.provenanceLabel):undefined,publicationId:row.publicationId?String(row.publicationId):undefined,publicationVersion:Number(row.publicationVersion||0),editorialStatus:'CREATOR_PUBLISHED' as const}];
 });
}

export function usePublishedLearningCards(chapterNumber:number){
 ...
 useEffect(()=>{
  ...
  fetch(`/api/knowledge?resource=published-learning-cards&chapter=${chapterNumber}`,{signal:controller.signal,headers:{Accept:'application/json'}})
