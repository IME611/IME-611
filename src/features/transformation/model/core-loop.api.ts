...
async function call(body:any){const r=await fetch('/api/core-loop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw new Error(d.error||'Core loop request failed');return d}
export const canonicalCoreLoopApi={
 ...
 async provenance(insightId:string):Promise<ProvenanceTrace>{const r=await fetch(`/api/core-loop?insightId=${encodeURIComponent(insightId)}`);const d=await r.json();if(!r.ok)throw new Error(d.error||'Provenance request failed');return d},
};
